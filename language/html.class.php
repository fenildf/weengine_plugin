...
class Sim_html
{
    ...
    public function gen_list($content){
        ...
        $weite_jsdk =$_W['siteroot'] . 'app' . str_replace("./","/",_MobileUrl('jsdk', array(), true)); 
        $tid = intval($_GPC['id']); 
        $list = pdo_fetch("SELECT * FROM ". tablename($this->modulename . '_ziyuan_list') . "WHERE tid = '$tid'");
        foreach($list as $class){
            $cid = $class['classid'];
        }

        $file = CACHE_ROOT . "/list" . $cid . ".html";
        $item = pdo_fetch("SELECT * FROM " . tablename($this->modulename . '_category') . " WHERE tid = '$cid ' and weid = '$weid' limit 1"); 
